...
const users = [
    ...
    { name: "고길동", image: "https://png.pngtree.com/png-clipart/20190630/original/pngtree-vector-avatar-icon-png-image_4162757.jpg" },
    { name: "신길동", image: "https://png.pngtree.com/png-clipart/20190520/original/pngtree-male-worker-icon-graphic-png-image_3668949.jpg" },
];
...
function CommentList(props) {
    return (
        <>
            <h1>홍길동만 출력</h1>
            {
                commentsAndUsers.filter(c => c.name === '홍길동').map((c, index) => <Comment key={index} name={c.name} comment={c.comment} image={c.image} />)
            }

            <h1>전체 출력</h1>
            {
                commentsAndUsers.map((c, index) => <Comment key={index} name={c.name} comment={c.comment} image={c.image} />)
            }
        </>
    );
}
...
![](Pasted%20image%2020230612150357.png)
